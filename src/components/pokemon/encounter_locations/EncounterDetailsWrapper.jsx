    ...
    <div className={styles.terrainWrapper}>
      {encounter.encounters.length === 0 ? (
        ...
        encounter.encounters.map((encounterLocation) => (
          <div key={encounterLocation.terrain} className={styles.terrainBlock}>
            <div
              className={`${styles.terrainSidebar} ${styles[`terrain${encounterLocation.terrain.charAt(0).toUpperCase() + encounterLocation.terrain.slice(1).toLowerCase()}`]}`}
            >
              <span>{encounterLocation.terrain}</span>
            </div>
            <div className={styles.terrainContent}>
