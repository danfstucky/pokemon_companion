...
const RadarChart = {
  draw(chartId, data, options) {
    ...
    const axis = radarGraph.selectAll('.axis').data(allAxis).enter().append('g').attr('class', 'axis');
    ...
    axis
      ...
      .attr(
        'x',
        (_, index) =>
          (cfg.w / 2) * (1 - cfg.factorLegend * Math.sin((index * cfg.radians) / total)) -
          60 * Math.sin((index * cfg.radians) / total)
      )
      .attr(
        'y',
        (_, index) =>
          (cfg.h / 2) * (1 - Math.cos((index * cfg.radians) / total)) - 20 * Math.cos((index * cfg.radians) / total)
      );

    const dataValues = [];
    data.forEach((y) => {
      radarGraph.selectAll('.nodes').data(y, (stat, index) => {
        dataValues.push([
          (cfg.w / 2) *
            (1 -
              (parseFloat(Math.max(stat.value, 0)) / cfg.maxValue) *
      ...
      radarGraph
        ...
        .on('mouseover', function () {
          ...
          radarGraph.selectAll('polygon').transition(200).style('fill-opacity', 0.1);
          radarGraph.selectAll(z).transition(200).style('fill-opacity', 0.7);
        })
        .on('mouseout', () => {
          radarGraph.selectAll('polygon').transition(200).style('fill-opacity', cfg.opacityArea);
        });
    });
    ...
    data.forEach((y) => {
      radarGraph
        ...
        .attr('cx', (stat, index) => {
          ...
          return (
            (cfg.w / 2) *
            (1 - (Math.max(stat.value, 0) / cfg.maxValue) * (cfg.factor * Math.sin((index * cfg.radians) / total)))
          );
        })
        .attr(
          'cy',
          (stat, index) =>
            (cfg.h / 2) *
            (1 - (Math.max(stat.value, 0) / cfg.maxValue) * (cfg.factor * Math.cos((index * cfg.radians) / total)))
        )
        ...
        .on('mouseover', function () {
          d3.select(this).style('fill', cfg.nodeColor).style('stroke-width', '3px');
        })
        .on('mouseout', function () {
          d3.select(this).style('fill', '#fff').style('stroke-width', '2px');
        })
        .append('svg:title')
    ...
    // Tooltip
    radarGraph.append('text').style('opacity', 0).style('font-family', 'sans-serif').style('font-size', '13px');
  },
};
